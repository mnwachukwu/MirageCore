...
| `NeedsAtLeast(string key, integer least)` | — | Offered only to a body carrying at least that much under that key. Below it the entry is grayed out instead of hidden, so a player can still see the verb exists. |

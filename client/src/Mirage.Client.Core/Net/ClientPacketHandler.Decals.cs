...
public sealed partial class ClientPacketHandler
{
    ...
    ///
    /// <para>A replace rather than a merge, because the server sent a whole list: stains merge as they
    ...
    private void HandleDecalUpdate(DecalUpdatePacket p)
    {
        ...
        foreach (var e in p.Decals)
        {
            ...
            decals.Add(new State.ClientState.Decal
            {
                ...
                Size = e.Size,
                Amount = amount,
                // Freshness is how much the stain has left of its last deposit, so a fresh one opens at full

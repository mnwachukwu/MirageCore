...
/// <summary>Ground stains: the generated stain atlas, the accumulation pass into an offscreen field, and
...
{
    // ── Ground stains ──────────────────────────────────────────────────────
    private Texture2D? _decalTex;
    private const int DecalVariants = 3;    // lumpy CORE variants in the atlas (the streak + droplet cells follow these)
    private const int DecalBlobSize = 96;   // px per atlas cell (resolution headroom for big draws)
    private const float DecalCoreRadius = 0.52f;   // central blob radius (normalized; 1 = cell half) — bigger core so pools read solid + merge
    ...
    private const int DropletMax = 30;         // droplet count at a FULL tile — ramps in lockstep with the blob size (shares DecalSizeFullAmount), so droplets ring the pool at every size, birth-anchored
    private const float DropletMinPx = 1f;     // droplet diameter range (px) — per-droplet, FIXED (independent of pool size)
    private const float DropletMaxPx = 4f;
    private const float DropletDistMinFrac = 0.30f; // droplet distance as a FRACTION of the pool size (min) — so droplets sit just past the rim at ANY size, not a fixed px that's too far on a small pool / swallowed by a big one
    private const float DropletDistMaxFrac = 0.48f; // + up to here, per-droplet — a tight cluster hugging the rim
    private const float DropletFadeFloor = 0.6f;  // droplets are fully GONE once the stain's freshness falls to this — so the spatter clears while the pool is still dark (raise toward 1 to clear it even sooner)
    private const float DropletFadePower = 2.5f;  // fade curve WITHIN the [floor..1] window — >1 so the spatter thins out faster than the pool, cueing a stain's freshness

    // One atlas texture holding, side by side: DecalVariants lumpy CORE blobs then a single DROPLET cell (a soft
    // dot), stamped as separate quads at draw time (see DrawDecalInfluence) and welded into one mass by the
    ...
    private void EnsureDecalTextures(GraphicsDevice gd)
    {
        if (_decalTex is not null) return;
        int sz = DecalBlobSize, cells = DecalVariants + 1, w = sz * cells;
        var px = new Color[w * sz];
        float c = sz / 2f;

        // Core variants: solid center, soft irregular rim (the lumpy blob that GROWS with the pool amount).
        for (int v = 0; v < DecalVariants; v++)
        {
            int ox = v * sz;
            for (int y = 0; y < sz; y++)
            {
                for (int x = 0; x < sz; x++)
                {
                    ...
                    float coreEdge = DecalCoreRadius * (1f + 0.16f * MathF.Sin(ang * 3f + v * 1.7f) + 0.10f * MathF.Sin(ang * 5f - v * 2.3f));
                    float a01;
        ...
        _decalTex = new Texture2D(gd, w, sz);
        _decalTex.SetData(px);
    }

    // Draws the frame's stains — a few stamps (core + streaks + droplets) per stained tile, all
    ...
    // Accumulate every stain into the offscreen decalRT with MAX blend: overlapping blobs take the
    ...
    // Two-layer world: accumulate only the pools on `layer` into `decalRT`, so ground stain and fringe (bridge-
    // top) stain build separate fields that composite in their own passes (fringe over ground).  Skips the whole
    // render-target swap when this layer has no stains, so a flat map pays nothing for the second field.
    private bool AccumulateDecalField(SpriteBatch sb, RenderTarget2D decalRT, Matrix transform, WorldLayer layer)
    {
        var decals = _renderFrame.Decals;
        bool any = false;
        for (int i = 0; i < decals.Count && !any; i++) if (decals[i].Layer == layer) any = true;
        if (!any) return false;

        EnsureDecalTextures(sb.GraphicsDevice);
    ...
    // Stamps one tile's stain into the MAX-blend accumulation buffer.  Every stain gets a CORE POOL BLOB
    ...
    private static float DecalHash(float n)
    {
        float s = MathF.Sin(n * 12.9898f) * 43758.5453f;
        return s - MathF.Floor(s);
    }
    private void DrawDecalInfluence(SpriteBatch sb, in DecalDrawCmd cmd)
    {
        var tex = _decalTex!;
        int sz = DecalBlobSize;
        float sat = Math.Clamp(cmd.Amount / Constants.DecalSizeFullAmount, 0f, 1f);
        float influence = cmd.Freshness;
        uint h = (uint)cmd.Seed;
        int variant = (int)(h % DecalVariants);
        float rot = ((h >> 3) % 360) * (MathF.PI / 180f);
        float jitter = 0.85f + ((h >> 12) % 31) / 100f;   // 0.85..1.15
        var tint = Color.White * influence;               // MAX-accumulate the influence; the stain color is applied at composite
        ...
        float coreSize = MathHelper.Lerp(Constants.DecalMinSizePx, Constants.DecalMaxSizePx, sat) * jitter * scale;
        ...
        // pool), so a droplet never slides as stain accumulates or dries.  The COUNT ramps with saturation, so new
        // droplets appear on top of the ones already placed; the newest fades in with `dPartial` (no pop).
        var dropSrc = new Rectangle(DecalVariants * sz, 0, sz, sz);
        // Droplets fade to NOTHING by the time the stain's freshness drops to DropletFadeFloor — well before
        ...
        float dropFade = Math.Clamp((influence - DropletFadeFloor) / (1f - DropletFadeFloor), 0f, 1f);
        var dropTint = Color.White * MathF.Pow(dropFade, DropletFadePower);
        float dCountF = DropletMax * sat;   // droplet count ramps in lockstep with the blob SIZE (shared `sat` = amount / DecalSizeFullAmount)
        ...
        for (int k = 0; k <= dWhole; k++)
        {
            ...
            float phi = rot + DecalHash(baseN) * MathF.Tau;
            ...
            float birthSize = MathHelper.Lerp(Constants.DecalMinSizePx, Constants.DecalMaxSizePx, k / (float)DropletMax) * jitter * scale;
            float dist = birthSize * (DropletDistMinFrac + DecalHash(baseN + 2.1f) * (DropletDistMaxFrac - DropletDistMinFrac));
            float dropPx = (DropletMinPx + DecalHash(baseN + 4.3f) * (DropletMaxPx - DropletMinPx)) * scale;  // 1..4 px, per-droplet (scaled up for a big NPC)
    ...
    // Composite the merged field over the ground as one tinted 1:1 quad. The viewport-sized dest rect maps
    // under the world transform to the full supersampled target, matching decalRT 1:1. Premultiplied world
    // tint * MaxAlpha, so the union reads as solid (with a soft rim from the blob falloff).
    private void CompositeDecalField(SpriteBatch sb, RenderTarget2D decalRT)
    {
        uint rgb = _ctx.State.DecalColor;
        var tint = new Color((int)((rgb >> 16) & 0xFF), (int)((rgb >> 8) & 0xFF), (int)(rgb & 0xFF)) * Constants.DecalMaxAlpha;
        sb.Draw(decalRT, new Rectangle(0, 0, Camera.ViewW, Camera.ViewH), tint);
    }
}
